...
function showBuddyReply(e){if(e)e.preventDefault();if(!buddy)return;unlockMusic();const text=buddyLines[buddyTap++%buddyLines.length];buddy.classList.remove('tapped');void buddy.offsetWidth;buddy.classList.add('tapped');document.querySelectorAll('.buddy-pop').forEach(x=>x.remove());const pop=document.createElement('div');pop.className='buddy-pop active';pop.textContent=text;document.body.appendChild(pop);const r=buddy.getBoundingClientRect();const x=Math.max(16,Math.min(innerWidth-16,r.left+r.width/2));const y=Math.max(16,r.top-10);pop.style.left=x+'px';pop.style.top=y+'px';pop.style.opacity='1';pop.style.transform='translate(-50%,0) scale(1)';if(!reduce){pop.animate([{opacity:0,transform:'translate(-50%,14px) scale(.78)'},{opacity:1,transform:'translate(-50%,0) scale(1.04)',offset:.16},{opacity:1,transform:'translate(-50%,-20px) scale(1)',offset:.68},{opacity:0,transform:'translate(-50%,-60px) scale(.95)'}],{duration:3000,easing:'cubic-bezier(.2,.8,.2,1)'}).finished.then(()=>pop.remove()).catch(()=>pop.remove())}else setTimeout(()=>pop.remove(),2200);setTimeout(()=>buddy.classList.remove('tapped'),650)}
if(buddy){buddy.addEventListener('click',showBuddyReply);buddy.addEventListener('pointerup',showBuddyReply);buddy.addEventListener('touchend',showBuddyReply,{passive:false})}
});